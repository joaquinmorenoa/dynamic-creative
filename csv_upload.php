...
if($_FILES['csvfile']['tmp_name']) {
    $upload = new Upload();
    $upload->SetFileName($_FILES['csvfile']['name']);
    $upload->SetTempName($_FILES['csvfile']['tmp_name']);
    $upload->SetUploadDirectory("csv/"); //Upload directory, this should be writable
    $upload->SetValidExtensions(array('csv')); //Extensions that are allowed if none are set all extensions will be allowed.
...
class Upload {
    ...
    function ValidateExistance()
    {
        ...
        if (file_exists($File)) 
        {
            ...
            $UniqueName = date("d-n-y") . '_' . $FileName;
